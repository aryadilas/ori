...
class DataVisualisation extends Page
{
    ...
    protected static ?string $navigationLabel = 'Peta Wilayah ORI';

    protected static ?string $slug = 'peta-wilayah-ori';

    protected static ?string $label = 'Peta Wilayah ORI';

    protected static ?string $pluralModelLabel = 'Peta Wilayah ORI';

    protected static ?string $navigationGroup = 'PELAPORAN';

    
    protected static ?string $title = 'Peta Wilayah ORI';

    protected ?string $heading = 'Peta Wilayah ORI';

    protected ?string $subheading = 'Halaman untuk melihat peta wilayah ORI.';
    ...
    public function childGenderData()
    {
        ...
        $this->childGender = Form2Answer::selectRaw('gender, COUNT(*) as total')
            ->when($this->kode_fasyankes, fn($q, $kode) => $q->where('kode_fasyankes', $kode))
    ...
    public function ChangeKodeFasyankes()
    {
        ...
        /* Grafik 4 */
        $this->graphic_4 = ViewSummarySckOri::when($this->kode_fasyankes, fn($q, $kode) => $q->where('kode_fasyankes', $kode))->where('year', $this->year)->get();

        /* Luas Wilayah */
        $this->luas_wilayah = Form1Answer::when($this->kode_fasyankes, fn($q, $kode) => $q->where('kode_fasyankes', $kode))
            ->where('year', $this->year)
            ->get()
            ->toArray();
    ...
    public function mount()
    {
        ...
        $this->kode_fasyankes = auth()->user()->kode_fasyankes;
